...
public:
    vector<int> anagramMappings(vector<int>& A, vector<int>& B) {
        vector<int> map;
        // Iterate over the collection A which will define our mapping.
        // We know that each character in A will be found somewhere in B
        // (at least once).
        for (int i = 0; i < A.size(); ++i)
            for (int j = 0; j < B.size(); ++j)
                // If we find a matching character in B, we have realized
                // a mapping.
                if (A[i] == B[j]) {map.push_back(j); break;}
        return map;
    }
};
